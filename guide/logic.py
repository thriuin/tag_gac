...
def organization_rule(cxt, org_name):
    ...
    org = cxt[org_name]
    try:
        data = models.Organization.objects.filter(name=org)
        for ta in cxt['ta']:
...
def value_threshold_rule(cxt, value_name, type_name):
    ...
    try:
        for ta in cxt['ta']:
            check = models.ValueThreshold.objects.filter(type=type).values_list(ta).get()[0]
            if value < check:
...
def code_rule(cxt, code_name, type_name, org_name):
    ...
    if str(type) == 'Goods':
        ...
        data = models.Code.objects.filter(code=value)
        for ta in cxt['ta']:
            cxt = check_if_trade_agreement_applies(ta, cxt, data, code_name)
